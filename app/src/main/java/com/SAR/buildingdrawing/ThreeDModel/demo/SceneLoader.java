package com.SAR.buildingdrawing.ThreeDModel.demo;
...
import com.SAR.buildingdrawing.ThreeDModel.ModelRenderer;
...
public class SceneLoader implements LoaderTask.Callback {
    ...
    public void init() {
        ...
        if (uri.toString().toLowerCase().contains(".obj") || parent.getParamType() == 0) {
            new WavefrontLoaderTask(parent, uri, this).execute();
        } else if (uri.toString().toLowerCase().contains(".stl") || parent.getParamType() == 1) {
            Log.i("Object3DBuilder", "Loading STL object from: "+uri);
            new STLLoaderTask(parent, uri, this).execute();
        } else if (uri.toString().toLowerCase().contains(".dae") || parent.getParamType() == 2) {
